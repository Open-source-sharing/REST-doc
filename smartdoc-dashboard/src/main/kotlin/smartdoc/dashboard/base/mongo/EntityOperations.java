...
public class EntityOperations {
  ...
  /**
   * Return the name used for {@code $geoNear.distanceField} avoiding clashes with potentially
   * existing properties.
   *
    ...
    try {
      ...
      if (ClassUtils.matchesTypeName(o_O.getClass(), "JSONParseException")) {
        throw new MappingException(
            "Could not parse given String to save into a JSON document!", o_O);
      }
      throw o_O;
    }
  }
  ...
  interface Entity<T> {
    ...
    /**
     * Returns the {@link Query} to remove an entity by its {@literal id} and if applicable
     * {@literal version}.
     *
    ...
    /**
     * Returns the value of the version if the entity {@link #isVersionedEntity() has a version
     * property}.
     *
     ...
     *     Make sure to check {@link #isVersionedEntity()}.
     */
    @Nullable
    Object getVersion();
  ...
  interface TypedOperations<T> {
    ...
    /**
     * Return the optional {@link Collation} from the given {@link Query} and fall back to the
     * collation configured for the underlying entity.
     *
     * @return
     */
  ...
  /**
   * {@link TypedOperations} for generic entities that are not represented with {@link
   * PersistentEntity} (e.g. custom conversions).
   */
