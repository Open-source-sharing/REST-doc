...
public final class ReflectUtils {
   ...
   * @param methodName Method signature, e.g.: method1(int, String). It is allowed to provide method
   *     name only, e.g.: method2
   * @return target method
   * @throws NoSuchMethodException
   * @throws ClassNotFoundException
   * @throws IllegalStateException when multiple methods are found (overridden method when parameter
   *     info is not provided)
   */
  public static Method findMethodByMethodSignature(
      Class<?> clazz, String methodName, String[] parameterTypes)
   ...
   *
   * <p>This method will not trigger classloading for the given interface, therefore it will not
   * lead to error when the given interface is not visible by the classloader
   *
  ...
  private static Object getEmptyObject(
      Class<?> returnType, Map<Class<?>, Object> emptyInstances, int level) {
    if (level > 2) {
      return null;
    }
    if (returnType == null) {
  ...
  public static Set<ParameterizedType> findParameterizedTypes(Class<?> sourceClass) {
    ...
    Set<ParameterizedType> parameterizedTypes =
        genericTypes.stream()
  ...
  /**
   * Find the hierarchical types from the source {@link Class class} by specified {@link Class
   * type}.
   *
   * @param sourceClass the source {@link Class class}
