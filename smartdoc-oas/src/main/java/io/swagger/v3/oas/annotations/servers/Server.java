 ...
 * io.swagger.v3.oas.annotations.Operation#servers()} to define servers for the single operation
 * (when applied at method level) or for all operations of a class (when applied at class level).
 *
 * <p>It can also be used in {@link io.swagger.v3.oas.annotations.OpenAPIDefinition#servers()} to
 * define spec level servers.
 *
 * @see <a target="_new"
 *     href="https://github.com/OAI/OpenAPI-Specification/blob/3.0.1/versions/3.0.1.md#serverObject">Server
...
public @interface Server {
   ...
   * indicate that the host location is relative to the location where the OpenAPI definition is
