...
/**
 * The annotation may be used to define a Schema for a set of elements of the OpenAPI spec, and/or
 * to define additional properties for the schema. It is applicable e.g. to parameters, schema
 * classes (aka "models"), properties of such models, request and response content, header.
 *
 * <p>swagger-core resolver and swagger-jaxrs2 reader engine consider this annotation along with
 * JAX-RS annotations, element type and context as input to resolve the annotated element into an
 * OpenAPI schema definition for such element.
 *
 * <p>The annotation may be used also to override partly (e.g. the name) or fully (e.g providing a
 * completely different representation) the schema of an element; for example if a specific class is
 * provided as value of {@link Schema#implementation()}, it will override the element type
 *
 * <p>The annotation {@link ArraySchema} shall be used for array elements; {@link ArraySchema} and
 * {@link Schema} cannot coexist
 *
 * @see <a target="_new"
 *     href="https://github.com/OAI/OpenAPI-Specification/blob/3.0.1/versions/3.0.1.md#schemaObject">Schema
...
public @interface Schema {
  /**
   * Provides a java class as implementation for this schema. When provided, additional information
   * in the Schema annotation (except for type information) will augment the java class after
   * introspection.
   *
  ...
  /**
   * Provides an array of java class implementations which can be used to describe multiple
   * acceptable schemas. If more than one match the derived schemas, a validation error will occur.
   *
  ...
  /**
   * Provides an array of java class implementations which can be used to describe multiple
   * acceptable schemas. If any match, the schema will be considered valid.
   *
  ...
  /**
   * Provides an array of java class implementations which can be used to describe multiple
   * acceptable schemas. If all match, the schema will be considered valid
   *
  ...
  /**
   * Sets the minimum numeric value for a property. Ignored if the value is an empty string or not a
   * number.
   *
  ...
  /**
   * Constrains the number of arbitrary properties when additionalProperties is defined. Ignored if
   * value is 0.
   *
  ...
  /**
   * Provides an optional override for the format. If a consumer is unaware of the meaning of the
   * format, they shall fall back to using the basic type without format. For example, if
   * \&quot;type: integer, format: int128\&quot; were used to designate a very large integer, most
   * consumers will not understand how to handle it, and fall back to simply \&quot;type:
   * integer\&quot;
   *
  ...
  /**
   * Sets whether a value should only be written to during a request but not returned during a
   * response.
   *
   * @deprecated As of 2.0.0, replaced by {@link #accessMode()}
   ...
   *
   * <p>AccessMode.READ_ONLY: value will not be written to during a request but may be returned
   * during a response. AccessMode.WRITE_ONLY: value will only be written to during a request but
   * not returned during a response. AccessMode.READ_WRITE: value will be written to during a
   * request and returned during a response.
   *
  ...
  /**
   * Provides an example of the schema. When associated with a specific media type, the example
   * string shall be parsed by the consumer to be treated as an object or an array.
   *
  ...
  /**
   * Provides an override for the basic type of the schema. Must be a valid type per the OpenAPI
   * Specification.
   *
