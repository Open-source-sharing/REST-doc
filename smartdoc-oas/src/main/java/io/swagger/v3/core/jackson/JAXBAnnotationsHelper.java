...
/**
 * The <code>JAXBAnnotationsHelper</code> class defines helper methods for applying JAXB annotations
...
class JAXBAnnotationsHelper {
  ...
  private static void applyElement(XmlElementWrapper wrapper, Schema property) {
    if (wrapper != null) {
      ...
      xml.setWrapped(true);
      // No need to set the xml name if the name provided by xmlelementwrapper annotation is
      // ##default or equal to the property name |
      // https://github.com/swagger-api/swagger-core/pull/2050
      if (!JAXB_DEFAULT.equals(wrapper.name())
          && !wrapper.name().isEmpty()
          && !wrapper.name().equals(property.getName())) {
   ...
   * @return <code>true</code> if the passed property can be represented as node attribute
   */
