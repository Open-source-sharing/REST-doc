...
public class ReflectionUtils {
  ...
  /**
   * Load Class by class name. If class not found in it's Class loader or one of the parent class
   * loaders - delegate to the Thread's ContextClassLoader
   *
  ...
  /**
   * Checks if the method methodToFind is the overridden method from the superclass or
   * superinterface.
   *
  ...
  public static boolean isOverriddenMethod(Method methodToFind, Class<?> cls) {
    ...
    for (Class<?> superClass : superClasses) {
      if (superClass != null && !(superClass.equals(Object.class))) {
        try {
          Method found =
              superClass.getMethod(methodToFind.getName(), methodToFind.getParameterTypes());
  ...
  public static boolean hasOverriddenMethods(Method methodToFind, Class<?> cls) {
    ...
    for (Method method : cls.getMethods()) {
      ...
      if (equalsMethodName
          && equalsParamCount
          && (superClassReturnAssignable || classReturnAssignable)) {
        ...
        for (int i = 0; i < paramsToFind.length; i++) {
          boolean superClassParamAssignable =
              method.getParameterTypes()[i].isAssignableFrom(paramsToFind[i]);
          boolean classParamAssignable =
              paramsToFind[i].isAssignableFrom(method.getParameterTypes()[i]);
  ...
  /**
   * Searches the method methodToFind in given class cls. If the method is found returns it, else
   * return null.
   *
      ...
      if (method.getName().equals(methodToSearch)
        ...
        if (soughtForParameterType.length == srcParameterTypes.length
          ...
          if (hasIdenticalParameters(
              srcParameterTypes,
              soughtForParameterType,
              srcGenericParameterTypes,
              soughtForGenericParameterType)) {
            return method;
          }
        }
      }
  ...
  private static boolean hasIdenticalParameters(
      Class<?>[] srcParameterTypes,
      Class<?>[] soughtForParameterType,
      Type[] srcGenericParameterTypes,
      Type[] soughtForGenericParameterType) {
    for (int j = 0; j < soughtForParameterType.length; j++) {
      Class<?> parameterType = soughtForParameterType[j];
      if (!(srcParameterTypes[j].equals(parameterType)
          || (!srcGenericParameterTypes[j].equals(soughtForGenericParameterType[j])
  ...
  public static boolean isConstructorCompatible(Constructor<?> constructor) {
    if (!Modifier.isPublic(constructor.getModifiers())) {
      ...
      return constructor.getParameterTypes().length == 0
          && (constructor.getDeclaringClass().getModifiers() & access)
   ...
   * excluding <code>Object</code> class. If the field from child class hides the field from
   * superclass, the field from superclass won't be added to the result list.
   *
   * <p>The list is sorted by name to make the output of this method deterministic. See
   ...
   */
  public static <A extends Annotation> List<A> getRepeatableAnnotations(
  ...
  public static <A extends Annotation> A[] getRepeatableAnnotationsArray(
      Class<?> cls, Class<A> annotationClass) {
      ...
      for (int i = 0; i < methodAnnotations.length; i++) {
        ...
        for (int j = 0; j < overriddenAnnotations[i].length; j++) {
          if (!types.contains(overriddenAnnotations[i][j].annotationType())) {
            methodAnnotations[i] =
                ArrayUtils.add(methodAnnotations[i], overriddenAnnotations[i][j]);
          }
        }
      }
    }
  ...
  public static boolean isSystemType(JavaType type) {
    // used while resolving container types to skip resolving system types; possibly extend by
    ...
    for (String systemPrefix : PrimitiveType.systemPrefixes()) {
      if (type.getRawClass().getName().startsWith(systemPrefix)) {
        if (!PrimitiveType.nonSystemTypes().contains(type.getRawClass().getName())
            && !PrimitiveType.nonSystemTypePackages()
                .contains(type.getRawClass().getPackage().getName())) {
          return true;
        }
      }
    }
