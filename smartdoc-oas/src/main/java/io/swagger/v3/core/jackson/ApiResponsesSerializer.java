...
public class ApiResponsesSerializer extends JsonSerializer<ApiResponses> {
  ...
  @Override
  public void serialize(ApiResponses value, JsonGenerator jgen, SerializerProvider provider)
      throws IOException {
    ...
    if (value != null && value.getExtensions() != null && !value.getExtensions().isEmpty()) {
      ...
      if (!value.isEmpty()) {
        for (Entry<String, io.swagger.v3.oas.models.responses.ApiResponse> entry :
            value.entrySet()) {
