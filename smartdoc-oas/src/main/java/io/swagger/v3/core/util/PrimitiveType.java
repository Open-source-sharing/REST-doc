...
/**
 * The <code>PrimitiveType</code> enumeration defines a mapping of limited set of classes into
...
public enum PrimitiveType {
  ...
  /** Adds support of a small number of "well-known" types, specifically for Joda lib. */
  ...
  /** Alternative names for primitive types that have to be supported for backward compatibility. */
  ...
  public static PrimitiveType fromTypeAndFormat(String type, String format) {
    ...
    return fromName(
        datatypeMappings.get(
            String.format(
                "%s_%s",
                StringUtils.isBlank(type) ? "" : type, StringUtils.isBlank(format) ? "" : format)));
  }
  ...
  /**
   * Convenience method to map LocalTime to string primitive with rfc3339 format partial-time. See
