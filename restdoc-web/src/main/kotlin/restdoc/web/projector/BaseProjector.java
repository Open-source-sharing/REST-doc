...
public abstract class BaseProjector<R> implements Projector<R> {
  ...
  static final Pattern ARRAY_PATTERN = compile("([a-zA-Z0-9_]+[a-zA-Z0-9_\\-]*)(\\[\\d*\\])+");

  // Filter the field
  private static final String FIELD_REGEX = "^[a-zA-Z0-9_]+[a-zA-Z0-9]*$";
  ...
  void buildForTreeNode(List<PathValue> pathValues) {
    ...
    List<Node> parentNodes =
        this.nodes.stream().filter(t -> t.getPath().matches(FIELD_REGEX)).collect(toList());
  ...
  protected void findChildren(Node parentNode) {
    String regexValue =
        String.format("^%s\\.[a-zA-Z_]+[a-zA-Z0-9_]*$", BaseProjector.escape(parentNode.getPath()));
    String regexArray =
        String.format("^%s(\\[\\d*\\])$", BaseProjector.escape(parentNode.getPath()));
   ...
   * <p>
   *
   * <p>Deduplication by {@link PathValue#getPath()}
   *
   * <p>TODO why Deduplication?
   *
   * <p>Review The resolve code
   */
  @VisibleForTesting
              ...
              if (matcher.find()) {
                ...
                List<String> indices =
                    IntStream.range(0, indexes.size())
                        .mapToObj(
                            index -> {
                              String suffixIndices =
                                  indexes.subList(0, index + 1).stream()
  ...
  static List<Integer> splitIndex(String indexString) {
    List<String> indexStrings =
        Arrays.stream(indexString.split("\\]")).map(t -> t.replaceAll("\\[", "")).collect(toList());

    return indexStrings.stream().map(t -> t.isEmpty() ? 0 : parseInt(t)).collect(toList());
  }
