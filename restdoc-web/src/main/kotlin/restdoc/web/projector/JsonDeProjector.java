...
public class JsonDeProjector implements DeProjector {
  ...
  static class Node {
    ...
    public Node(String path, String field, FieldType type, FieldType parentType) {
      this.path = path;
      this.field = field;
      this.type = type;
            ...
            .map(
                node ->
                    new BodyFieldDescriptor(
                        ...
                        node.value))
            .filter(
                d -> !d.getType().equals(FieldType.ARRAY) && !d.getType().equals(FieldType.OBJECT))
  ...
  private void translate() {
    if (jsonNode instanceof ObjectNode) {
      this.translateObject(
          new Node("root", "root", FieldType.OBJECT, FieldType.MISSING), (ObjectNode) jsonNode);
  ...
  private void translateArray(Node parentNode, ArrayNode an) {
    for (int index = 0; index < an.size(); index++) {
      ...
      if (node instanceof ObjectNode) {
        Node childNode = new Node(path, null, FieldType.OBJECT, FieldType.ARRAY);
        ...
        Node childNode = new Node(path, null, FieldType.ARRAY, FieldType.ARRAY);
        ...
        Node childNode = new Node(path, null, FieldType.MISSING, FieldType.ARRAY);
  ...
  private void translateObject(Node parentNode, ObjectNode on) {
    ...
    while (fields.hasNext()) {
      ...
      if (node instanceof ObjectNode) {
        Node childNode =
            new Node(
                format("%s.%s", parentNode.path, field), field, FieldType.OBJECT, FieldType.OBJECT);
        ...
        Node childNode =
            new Node(
                format("%s.%s", parentNode.path, field), field, FieldType.ARRAY, FieldType.OBJECT);
