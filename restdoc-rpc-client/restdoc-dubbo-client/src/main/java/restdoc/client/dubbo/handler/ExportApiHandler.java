...
public class ExportApiHandler implements NettyRequestProcessor {
  ...
  public RemotingCommand processRequest(ChannelHandlerContext ctx, RemotingCommand request)
    ...
    List<DubboApiDescriptor> apiDescriptors =
        beansOfType.values().stream()
            .map(
                serviceBean -> {
                  refBeanManager.addRefBean(
                      serviceBean.getRef().getClass().getName(), serviceBean.getRef());
                  ...
                  List<DubboApiDescriptor.ExposedMethod> exposedMethods =
                      sd.getAllMethods().stream()
                          .map(
                              mh ->
                                  new DubboApiDescriptor.ExposedMethod(
                                      mh.getParamDesc(),
                                      mh.getCompatibleParamSignatures(),
                                      Arrays.stream(mh.getParameterClasses())
                                          .map(Class::getName)
                                          .toArray(String[]::new),
                                      mh.getParameterNames(),
                                      mh.getReturnClass().getName(),
                                      Arrays.stream(mh.getReturnTypes())
