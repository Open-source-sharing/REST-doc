...
public class DubboInvokerImpl implements Invoker<DubboInvocation> {
  ...
  public InvocationResult rpcInvoke(DubboInvocation invocation) {
    ...
    try {
      Class[] paramTypes =
          invocation.getParameters().stream()
              .map(
                  pa -> {
                    try {
      ...
      Object[] parameters =
          invocation.getParameters().stream().map(this::mappingValue).toArray(Object[]::new);
      ...
      String serializedReturnValue =
          returnValue == null ? "" : mapper.writeValueAsString(returnValue);
      return new DubboInvocationResult(
          false, null, invocation, serializedReturnValue, method.getReturnType().toString());

    } catch (Exception e) {
      e.printStackTrace();
      return new DubboInvocationResult(
          false, e.getMessage(), invocation, "", Void.class.toString());
    }
  }
