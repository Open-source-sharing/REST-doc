...
/**
 * The class AgentImpl default implement agent Provide remoting client and config
 *
 * @author Maple
 * @since 2.0.RELEASE
 */
public class AgentImpl implements Agent {
  ...
  public AgentImpl(ServerProperties serverProperties) {
    ...
    RemotingTask acknowledgeVersionTask =
        new RemotingTask(
            ackVersionTaskId,
            RemotingTaskType.SYNC,
            createRequestCommand(RequestCode.AcknowledgeVersion, null),
            3000,
            null);
  ...
  @Override
  public Boolean acknowledgeVersion()
    ...
    if (invokeResult != null
        && (response = invokeResult.getResponse()) != null
  ...
  @Override
  public InvokeResult invoke(String taskId)
      throws NoSuchElementException, InterruptedException, RemotingTooMuchRequestException,
          RemotingTimeoutException, RemotingSendRequestException, RemotingConnectException {
    RemotingTask remotingTask =
        remotingTasks.stream()
            .filter(t -> t.getTaskId().equals(taskId))
            .findFirst()
            .orElseThrow(
                () -> new NoSuchElementException(String.format("NoSuch Task id %s", taskId)));
    return this.invoke(remotingTask);
  }

  @Override
  public InvokeResult invoke(RemotingTask remotingTask)
      ...
      case ASYNC:
        {
          this.getRemotingClient()
              .invokeAsync(
                  ...
                  remotingTask.getRequest(),
                  remotingTask.getTimeoutMills(),
                  remotingTask.getInvokeCallback());
