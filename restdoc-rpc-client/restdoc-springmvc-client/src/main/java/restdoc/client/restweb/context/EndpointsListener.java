 ...
 *
 * <p>Springboot应用启动需要做的任务有 1 如果console端已经同步过了当前应用的API信息之后，则对已经存在的API文档进行记录建议开发者进行补充文档 2 如果{@link
 * restdoc.client.api.ServerProperties} 的service属性为空，则将当前应用的service补充维应用的ContextPath(去掉第一个"/") 3
 * 当透析一个API时，为每个API文档生成一份模板数据(测试用例) 仅为模板数据，并不准确 4
 * 导入的API的唯一ID为(controller+function).hashcode().toString() 5
 * 开发者可以将console端已有的API文档和应用内部已有的API可以做关联(关联关系可以理解为所属) 6 所有的API文档具有关联性(API之间具有依赖性) 7
 * 如果部分API文档是通过应用生成的 则console端则应用增加标记图标，加强开发辨识度 8 应用SDK代码全部更改为java 去掉kotlin
 *
 * @author Maple
 * @since 1.0.RELEASE
 */
public class EndpointsListener implements ApplicationListener<ContextRefreshedEvent> {
                  ...
                  return requestMappingInfo.getPatternsCondition().getPatterns().stream()
                      ...
                      .map(
                          pattern -> {
                            ...
                            if (methods.isEmpty())
                              emptyTemplate.setMethod(RequestMethod.POST.name());
                            else if (methods.size() == 1)
                              emptyTemplate.setMethod((new ArrayList<>(methods)).get(0).name());
                            else if (methods.contains(RequestMethod.GET))
                              emptyTemplate.setMethod(
                                  methods.stream()
                                      .max(Enum::compareTo)
                                      .map(Enum::name)
                                      .orElse(RequestMethod.POST.name()));

                            emptyTemplate.setPackageName(
                                handlerMethod.getBeanType().getPackage().getName());
   ...
   * @see MultipartFile
   */
  private void projectApi(
      HttpApiDescriptor emptyTemplate,
      RequestMappingInfo requestMappingInfo,
      HandlerMethod handlerMethod) {

    emptyTemplate.setName(
        requestMappingInfo.getName() == null
            ? handlerMethod.getMethod().getName()
            : requestMappingInfo.getName());

    MethodParameter[] parameters = handlerMethod.getMethodParameters();

    boolean hasRequestPartAnnotation =
        Stream.of(parameters)
            .flatMap(t -> Stream.of(t.getParameterAnnotations()))
            .anyMatch(t -> t.annotationType() == RequestPart.class);

    boolean hasFileParam =
        Stream.of(parameters).anyMatch(t -> ResolverUtil.isFileType(t.getParameterType()));
    ...
    List<HttpApiDescriptor.ParameterDescriptor> requestMimeTypeDescriptor =
        emptyTemplate
            .getRequestHeaderParameters()
    ...
    if (!consumesCondition.isEmpty() && !requestMimeTypeDescriptor.isEmpty()) {
      ...
      Set<HttpApiDescriptor.ParameterDescriptor> pds =
          mts.stream()
              .map(
                  t -> {
      ...
      // setup fixed
      HttpApiDescriptor.ParameterDescriptor pd = new HttpApiDescriptor.ParameterDescriptor();
    ...
    emptyTemplate
        .getRequestHeaderParameters()
        .put(HttpHeaders.CONTENT_TYPE, requestMimeTypeDescriptor);
