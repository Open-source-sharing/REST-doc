...
final class CookieValueResolver implements Resolver {
  ...
  public void resolve(
    ...
    HttpApiDescriptor.KeyValuePair pair =
        emptyTemplate.getRequestHeaders().stream()
            .filter(t -> HttpHeaders.COOKIE.equals(t.getName()))
            .findFirst()
            .orElseGet(
                () -> {
                  HttpApiDescriptor.KeyValuePair newPair = new HttpApiDescriptor.KeyValuePair();
    ...
    pair.setDefaultValue(
        String.join(
            "; ", cookieValueString, String.format("%s=%s", name, cookieValue.defaultValue())));
  }
}
