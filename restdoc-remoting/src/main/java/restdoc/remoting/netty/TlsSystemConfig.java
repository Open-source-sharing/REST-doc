...
public class TlsSystemConfig {
   ...
   * authentication. This configuration item really only applies when building the server-side
   * {@link SslContext}, and can be set to none, require or optional.
   */
   ...
   *
   * <ol>
   *   <li><strong>disabled:</strong> SSL is not supported; any incoming SSL handshake will be
   *       rejected, causing connection closed.
   *   <li><strong>permissive:</strong> SSL is optional, aka, server in this mode can serve client
   *       connections with or without SSL;
   *   <li><strong>enforcing:</strong> SSL is required, aka, non SSL connection will be rejected.
   * </ol>
   */
  ...
  public static String tlsConfigFile =
      System.getProperty(TLS_CONFIG_FILE, "/etc/rocketmq/tls.properties");
}
